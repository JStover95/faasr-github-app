...
} from "../../functions/_shared/workflow-status-service.ts";
...
Deno.test("getWorkflowStatus - should sanitize file name", async () => {
  ...
  const sanitizeStub = stub(deps, "sanitizeFileName", (_name: string) => {
    sanitizeCalled = true;
...
Deno.test("getWorkflowStatus - should get most recent workflow run", async () => {
  ...
  const getRunStub = stub(
    deps,
    "getWorkflowRunById",
    async (_octokit: unknown, _owner: string, _repo: string, runId: number) => {
      capturedRunId = runId;
